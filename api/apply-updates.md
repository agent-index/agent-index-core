...
1.5. **Resolve the read base — id-anchored only (core 3.22.0, Option A; supersedes the core 3.10.1 dual-path).** Read `org-config.json` → `installed_collections[]`, find the entry for `<collection>`. If it has a non-empty `folder_id`, set `base = "id:{folder_id}"`. **The legacy `/{collection}` path read has been REMOVED as the default** (`memberapplynotdistaware` — Option A: one read path, one authority). If `folder_id` is **absent or empty**:
   - This is a pre-backfill org (folder IDs are captured admin-side at install and backfilled by Step 1.5 Migration 4). Do **not** silently read `/{collection}` — the silent fallback is exactly the split-brain we removed. Instead, surface an **explicit, logged** notice: `"{collection}: no folder_id in org-config — this org predates id-anchored distribution. Skipping this collection's file sync until the admin re-publishes (@ai:publish-updates) so Migration 4 backfills folder IDs."` and **SKIP this collection** (leave `manifest_sync[<collection>]` unadvanced so a later run retries once backfilled). A new install (incl. customer B) always has `folder_id`, so this branch never fires for them.
   - Reserved explicit fallback: a legacy read of `/{collection}` is permitted **only** when invoked behind an explicit, version-gated escape hatch (an operator setting / migration path), and when used it MUST emit the deprecation warning and be logged — it is never the default.

   All reads in steps 2–3 use `{base}/...`. **Rationale:** non-admin members are not Drive members and cannot reliably resolve a root-relative collection path (`aifs_list("/")` is unavailable to them; same-named folders are ambiguous — bug 20260606-62a14c43-230135-db13). Addressing by the collection's stored Drive ID makes the read deterministic and immune to name collisions.
...
1.6. **Config-validity gate before any bulk write (core 3.22.0 — closes `applyerrorpollution`).** Before issuing the parallel reads/writes below, validate that `agent-index.json` and the just-read `org-config.json` parse as complete JSON, and that the first `aifs_*` probe returns a well-formed envelope (not a `CONFIG_ERROR`/`NOT_AUTHENTICATED`/parse error). **If any `aifs_*` call returns an error envelope or unparseable output at any point in this subroutine, ABORT the whole subroutine immediately — do not write the payload anywhere.** Historically (member apply session, ms_prod_9) a mount served a truncated `agent-index.json` (missing its final `}`), every `aifs-exec` call then returned `CONFIG_ERROR`, and the error text was **written into all 38 manifest files**. Guard rules, mandatory:
   ...
   - **Bound the parallelism:** issue the per-capability reads in **small batches (≤ 4 concurrent)**, not an unbounded fan-out — the high-volume parallel run amplified the mount-tear exposure. After a transient mount/config error, the safe recovery is to abort, re-validate config (re-read `agent-index.json` and confirm it parses), and let the member re-run `@ai:update`; do **not** push through and pollute files.
...
3. If the member accepts: install capabilities by reading definitions from the **org remote copy** (the admin already placed the collection there via install-collection) — NEVER by re-downloading from GitHub/marketplace, and NEVER by writing collection files to the org remote. Specifically:
   - **Resolve the read base — id-anchored only (core 3.22.0, Option A).** Read `org-config.json` → `installed_collections[]` for this collection. If it has a `folder_id`, `base = "id:{folder_id}"`. The silent `/{collection}` legacy fallback has been **removed** (one read path, one authority); if `folder_id` is absent, fall through to the unreadable-base handling below rather than guessing a root-relative path. Read `coll = aifs_read("{base}/collection.json")`.
