...
4. Update the local `agent-index.json` version field. Also migrate any new top-level fields that are present in the canonical workspace template (read from remote) but missing locally. The canonical template lives at `agent-index-core/templates/agent-index.template.json` as of core 3.4.0; older installs may still find it at the legacy path `agent-index-core/agent-index.json` (which is kept at remote for one release for migration safety, removed in 3.5.0). When reading, try `aifs_read("/agent-index-core/templates/agent-index.template.json")` first; if that returns NOT_FOUND, fall back to `aifs_read("/agent-index-core/agent-index.json")`. Specifically as of 3.1.1: if `infrastructure_directory_url` is missing locally, copy it from the canonical template — without it, `check-updates` cannot determine the latest core or marketplace version. Field migrations are non-destructive: never overwrite a field the member has already set, only add fields that are absent.
5. **Clean up deprecated v2 artifacts:** If `agent-index-core/tools/aifs-bridge/` exists locally, delete the entire directory (it contains `aifs-bridge.mjs` and `aifs-call.sh` which reference the removed `server.bundle.js`). Also delete `mcp-servers/filesystem/server.bundle.js` if present. These are pre-v3 artifacts that cause errors if Claude discovers and tries to use them.
   ...
   **Delete legacy collection-template `agent-index-core/agent-index.json`** (added in core 3.4.0): this file shipped at the collection root in core ≤3.3.1 as a workspace bootstrap template. In 3.4.0 the canonical template moved to `agent-index-core/templates/agent-index.template.json`; the legacy path is kept at remote during 3.4.0 only for migration safety and is removed in 3.5.0. Locally the duplicate is harmless but confuses any tool that walks up looking for a workspace marker (the workspace's own `agent-index.json` lives at the install root, not under `agent-index-core/`). Concretely: if `<project_dir>/agent-index-core/agent-index.json` exists, delete it. The workspace's own `<project_dir>/agent-index.json` is the canonical workspace marker and must not be touched.
