...
5. **Write each file to the member-local install path(s).** The canonical install layout is the collection-qualified `members/{member_hash}/installed/{collection}/{type}/{name}/` — set `{install_dir}` to this. The `{collection}` segment (added for bug `instdir`) prevents same-named capabilities from different collections from colliding on `installed/{type}/{name}/`; it is derivable here because each member-index entry records its `collection`. **Old-unqualified migration:** if a pre-`instdir` directory exists at the unqualified `members/{member_hash}/installed/{type}/{name}/`, move it to `{install_dir}` before writing (or, if `{install_dir}` already has content, archive the unqualified directory to a timestamped backup) so no stale duplicate is left behind — this piggybacks on the normal resync write and needs no separate migration script. Separately, while the pre-3.6.x legacy layout `members/{member_hash}/{type}/{name}/` still exists on a given install, continue writing to it too to keep it in sync (that single-layout migration is tracked separately and is not in scope here). Specifically write:
   ...
   **5a. Materialize the collection's bundled scripts (added in core 3.29.0).** Once per collection — not once per capability — after the per-capability writes above and before step 8 advances the sync fields:
   ...
   - Otherwise copy `{base}/apps` to `members/{member_hash}/installed/{collection}/apps/`, preserving nested structure, LF-normalizing text content per step 4. Re-resolve the core-injected `apps_path` for this collection's capabilities to the absolute `{project_dir}/members/{member_hash}/installed/{collection}/apps` (`standards.md`, "Core-Injected Parameters") — `project_dir` differs between machines and Cowork mounts, so a value stored at original install time may no longer resolve. This is a **wholesale replacement**: remove the local directory first, then write. `apps/` is collection-owned and core-managed; nothing else writes there, so there is nothing to preserve and no merge to perform.
   ...
   **5b. Record `apps_path` in each affected capability's setup-responses (added in core 3.29.1 — closes `appspathnotbackfilled`).** Materializing the directory without recording the parameter that points at it leaves `{apps_path}` unresolved on every install that predates 3.29.0 — the scripts are present and still uncallable. `org-setup` Phase 4a/4b writes `apps_path` for a *fresh* install; 3.29.0 shipped no equivalent for the *migration* path, so existing members received the files and nothing else. This step is that equivalent.
   ...
   For each capability of this collection in `member-index.json`, write `apps_path` into `members/{member_hash}/installed/{collection}/{type}/{name}/setup-responses.md` under the `## Org-Mandated Parameters` section, value `{project_dir}/members/{member_hash}/installed/{collection}/apps`:
   ...
   - **Re-resolve, never carry forward.** The recorded value is a record of the last resolution, not an input to the next one (`standards.md`, "Core-Injected Parameters"). `project_dir` differs between machines and Cowork mounts, so this step recomputes the absolute path on every run and overwrites a stale one.
...
9. **Reconcile setup-responses org-mandated values** (added in core 3.7.6 to close the data half of bug `20260527-8d20ea22-4`). For each capability that was just resynced in steps 2–7, check whether its local `setup-responses.md` org-mandated values match the org's authoritative `collection-setup-responses.md` (read via `aifs_read("/{collection}/setup/collection-setup-responses.md")` — note the corrected path; the pre-3.7.6 `org-setup` spec had this wrong, which is why this reconciliation is needed on every existing install). If drifted: re-inject the org-mandated values into the local setup-responses, preserving member-defined and role-suggested values.
   ...
   **First-run behavior on existing installs (core 3.29.1).** An install that already applied 3.29.0 sits at revision 5 with `apps/` materialized but no `apps_path` recorded; the bump from 5 to 6 re-sweeps it, and step 5b writes the parameter. Step 5a is then a cheap no-op — the directory is re-copied and byte-identical. An install still on 3.28.x or earlier gets 5a and 5b together in one pass.
