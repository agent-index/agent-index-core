...
2. **Verify bundle integrity BEFORE overwriting anything — and do not defer on the sandbox proxy notice.** The fetch runs through the sandbox's transport proxy, so an `[aifs] Proxy detected — added CA certificate from …` line is **expected, benign, and not a trust signal** — it is documented adapter behavior (since v2.0.2) and prints on every aifs-exec call. Bundle authenticity is established by the **checksum gate below, not by transport trust**, so proceed through these checks rather than pausing on the proxy notice:
   - **Checksum gate (required).** Compute the fetched `aifs-exec.bundle.js`'s SHA-256 and compare it to the `exec_bundle_checksum` declared in the target `adapter.json` (which itself comes from the trusted org backend). On mismatch: **abort, do not install**, surface "adapter bundle checksum mismatch — expected `<declared>`, got `<actual>`; this is a security failure, not retrying. Report to your admin." A matching hash defeats any in-transit tampering regardless of the proxy.
   - **Embedded-URL / domain scan (required).** Scan the bundle for outbound endpoints and confirm they fall within the adapter's declared `required_domains`. Any unexpected domain aborts the install with the same fail-loud posture.
   - Only after both checks pass, continue to step 3.
3. Extract and overwrite `mcp-servers/filesystem/aifs-exec.bundle.js` and `aifs-exec.sh` and `adapter.json`.
4. **Sync `agent-index.json`'s `remote_filesystem.exec.adapter_version`** (added in core 3.7.2; closes idea `bundle-vs-config-adapter-drift`). After step 3 writes the new `adapter.json`, parse the freshly-installed `mcp-servers/filesystem/adapter.json` and read its `version` field. If `agent-index.json`'s `remote_filesystem.exec.adapter_version` differs from this value, rewrite `agent-index.json` to bring them into agreement. The `adapter.json` `version` is the authoritative source — it ships with the bundle and is what the bundle implementation actually exposes; `agent-index.json`'s field is denormalized metadata that historically wasn't kept in sync. Idempotent: same-value no-op. Without this step, the two files can drift indefinitely (the bundle gets updated by this phase but the config field stays at install-time value), which breaks downstream code that reads `agent-index.json` to gate behavior on the adapter version.
5. **Post-install smoke test (required).** Run `aifs_auth_status` against the newly installed bundle to confirm it loads and authenticates. If it fails, surface the failure and stop before advancing the cursor (leave the pending plan in place for a retry) — do not report the bundle as installed.
6. Write a pending plan file at `.agent-index/install-state/pending-update-plan.json` containing the remaining operations (collection upgrades, installs, etc.) and the `target_cursor`.
7. Surface: "Adapter bundle updated to {target_version} (checksum-verified). The new executor bundle is ready to use immediately. Say '@ai:update' and I'll continue with the remaining updates."
8. Continue to Phase 4 in this session.

> **Why not defer on the proxy notice (mitmcadefer, C.1.3.5).** A member applying unattended previously stalled at cursor n-1 because the flow treated the expected `[aifs] Proxy detected` CA notice as a reason to pause the code-bundle update. Integrity here is the checksum gate above (hash vs. the backend-declared `exec_bundle_checksum`), which is unaffected by the transport proxy — so the proxy notice alone must never block the bundle step. The verification (checksum / domain scan / smoke test) is mandatory and stays; only the unwarranted deferral is removed. Closes bug `20260630-8d20ea22-mitmcadefer`.
...
**Subroutine revision constant.** The manifest-sync subroutine (defined below) has a `CURRENT_SUBROUTINE_REVISION` constant that bumps whenever a step is added or its data-shape semantics change. As of core 3.29.2 this constant is **`7`** (revision 7 changed step 5b's stored form from an absolute path to one relative to `project_dir` — `appspathsandboxleak`; without this bump, an install that already advanced to revision 6 under 3.29.1 would keep a recorded value naming a Cowork sandbox that no longer exists). Prior: as of core 3.29.1 this constant was **`6`** (revision 6 added step 5b — record the core-injected `apps_path` in each affected capability's setup-responses). Prior: as of core 3.29.0 this constant was **`5`** (revision 5 added step 5a — materialize the collection's bundled `apps/` directory into the member-local install layout). Prior: as of core 3.10.1 this constant was **`4`** (revision 4 added step 1.5 — id-anchored read base via the collection's stored `folder_id`, falling back to the legacy `/{collection}` path; this forces a one-time re-sync per collection so reads migrate to id-anchored addressing wherever a `folder_id` is present). Prior: as of core 3.7.6 this constant was **`3`** (revision 1 was the 3.6.1 release with steps 1–6 and step 8; revision 2 added step 7 in 3.7.0, which reconciles `member-index.installed[].version` with the `.md` frontmatter version; revision 3 added step 9 in 3.7.6, which reconciles setup-responses org-mandated values against `collection-setup-responses.md` at the corrected path — closes the data half of bug `20260527-8d20ea22-4`). Phase 4.5 uses this constant alongside `manifest_sync[C]` to decide when to re-run the subroutine — this protects against the structural bug `20260512-8d20ea22` where a 3.7.0 install with already-populated `manifest_sync` (from a 3.6.1+ backfill) had the new step 7 silently skipped.
...
4. For each collection `C` in the effective set:
   ...
   **4.1. Filesystem-existence sub-check for collection C** (added in core 3.7.3). Verifies on-disk presence of the canonical install layout, not just the `manifest_sync` bookkeeping field. Catches two known drift classes: (a) pre-3.6.x installs whose capability files exist only at the legacy path (`members/{member_hash}/{type}/{name}/`) and were never backfilled to the canonical path (`members/{member_hash}/installed/{collection}/{type}/{name}/`, collection-qualified as of bug `instdir`) because dual-write only fires on install/upgrade; (b) any flow that advances `manifest_sync` without writing local files (bookkeeping-without-files state).
   ...
   5. **Bundled-scripts sub-check (added in core 3.29.0).** If collection C ships bundled scripts (`aifs_exists("/{C}/apps")` is true), also stat `members/{member_hash}/installed/{C}/apps/` locally. If the directory is absent, set `appsMissing` — a drift condition in its own right, independent of `canonicalAnchorsMissing`. This is the third and now most common drift class on this install base: every member who installed a collection before core 3.29.0 has correct capability files and no `apps/` at all, and without this check the sweep reports the layout complete while every `{apps_path}` invocation in that collection fails. The same "unknown on non-not-found errors" rule in item 4 applies.
   ...
   **4.2. Notice format on filesystem-drift detection** (added in core 3.7.3). When `canonicalAnchorsMissing` is what triggers the drift classification, surface this line in the apply-updates progress narration immediately before invoking the subroutine:
   ...
   When `appsMissing` is what triggers the classification (with or without missing anchors), surface instead:

   > "Collection `{C}` bundles scripts that aren't on this machine yet (`installed/{C}/apps/`). Materializing them now."
...
1.5. **Resolve the read base — path-primary, id-anchor fallback (core 3.22.2, J0; corrects the 3.22.0 Option-A regression).** Read `org-config.json` → `installed_collections[]`, find the entry for `<collection>`. Set the read base to the **collection path `/{collection}`** as the primary, and `id:{folder_id}` (if a non-empty `folder_id` is recorded) as the fallback:
   - **Primary: `base = "/{collection}"`** — the collection-root path. This is the read `org-setup` already uses for the same files, and it is the read that works for a non-admin member on **both** backends: the all-members group holds `reader` on each collection root (install-collection cr01 / Migration 4), so the path is enumerable; and on OneDrive the path resolves against the configured **site drive**, which is where collection files live.
   - **Why this is the correction:** 3.22.0 (Option A / `memberapplynotdistaware`) made `id:{folder_id}` the *only* base. On OneDrive a **bare `id:{folder_id}` resolves against the member's OWN drive (`/me/drive`), not the site drive** where the collection lives, so every member collection read 404'd (`i1onedrivebreak`, ms_prod_9). The `/{collection}` path is not "legacy" — it is the correct site-drive read. Restoring it as primary fixes OneDrive and keeps gdrive working (org-setup proves the path read on both backends).
   - **Fallback: `id:{folder_id}`** — if the path read in steps 2–3 fails (read error / PATH_NOT_FOUND) AND a non-empty `folder_id` is recorded, retry that read against `id:{folder_id}`. This is the gdrive determinism/enumeration safety net (and on gdrive a bare id-anchor resolves globally). If both the path and the id-anchor fail (or no `folder_id` to try), treat the collection as unreadable: surface a logged notice (`"{collection}: not readable via /{collection} or id:{folder_id} — ask your admin to (re-)publish / grant access"`) and **skip** it (leave `manifest_sync[<collection>]` unadvanced so a later run retries). Never silently conclude success on an unread collection.

   All reads in steps 2–3 use `{base}/...` (the primary), falling back per above. **Note (cross-backend):** the bare `id:{folder_id}` anchor is backend-sensitive — global on gdrive, own-drive-scoped on OneDrive — which is exactly why the path is primary. (A future unification could use the qualified cross-drive anchor `id:{site_drive_id}:{folder_id}` on OneDrive; tracked separately, not required here.)

1.6. **Config-validity gate before any bulk write (core 3.22.0 — closes `applyerrorpollution`).** Before issuing the parallel reads/writes below, validate that `agent-index.json` and the just-read `org-config.json` parse as complete JSON, and that the first `aifs_*` probe returns a well-formed envelope (not a `CONFIG_ERROR`/`NOT_AUTHENTICATED`/parse error). **If any `aifs_*` call returns an error envelope or unparseable output at any point in this subroutine, ABORT the whole subroutine immediately — do not write the payload anywhere.** Historically (member apply session, ms_prod_9) a mount served a truncated `agent-index.json` (missing its final `}`), every `aifs-exec` call then returned `CONFIG_ERROR`, and the error text was **written into all 38 manifest files**. Guard rules, mandatory:
   - **Never persist a payload that parses as an aifs error envelope** (has an `error`/`code` shape, or contains `CONFIG_ERROR`/`NOT_AUTHENTICATED`/`PATH_NOT_FOUND` markers) or that fails the file's own format check (e.g. a `.json` artifact that doesn't `JSON.parse`). Treat any such read as a hard failure for that capability and abort.
   - **Validate-then-write:** every file written in step 5 must first pass a shape check (`.json` parses; `.md` is non-empty and not an error string). A read that fails the check aborts before the write.
   - **Bound the parallelism AND retry transient reads (J4, `aifsreadflaky`):** issue the per-capability reads in **small batches (≤ 4 concurrent)**, not an unbounded fan-out — the high-volume parallel run amplified the mount-tear exposure. **Concurrent `aifs_read` on OneDrive intermittently fails under load even at ≤4** (observed: a single file's read blipping repeatedly, ms_prod_9). So a transient read failure (network/throttle/empty result — NOT a parsed error envelope) is **retryable**: retry the individual read up to 3× with a small backoff (e.g. 0.5s, 1s, 2s) before treating it as a real failure. Keep the two cases distinct: a **transient read failure → retry** (then skip the collection if still failing, unadvanced); a **CONFIG_ERROR / error-envelope / unparseable payload → hard-abort the whole subroutine** (the applyerrorpollution guard — never write it, never push through). Do not conflate the two: don't abort the run on a retryable blip, and don't retry-then-persist an actual error payload.

2. **Read the three canonical files from remote** for each capability (issued in **bounded batches of ≤ 4 concurrent per the 1.6 gate**, not an unbounded fan-out). Every read is checked against the 1.6 guard before use:
...
3. **Read the collection.json once** for this collection: `coll = aifs_read("{base}/collection.json")`. Use `coll.version` as the authoritative collection version for the manifest field sync below. (This is the value the org has on remote, which equals `targetVersion` in steady state.) Apply the 1.6 guard: if `coll` doesn't parse, abort this collection — do not proceed to write with a malformed `coll`.
...
5. **Write each file to the member-local install path(s).** The canonical install layout is the collection-qualified `members/{member_hash}/installed/{collection}/{type}/{name}/` — set `{install_dir}` to this. The `{collection}` segment (added for bug `instdir`) prevents same-named capabilities from different collections from colliding on `installed/{type}/{name}/`; it is derivable here because each member-index entry records its `collection`. **Old-unqualified migration:** if a pre-`instdir` directory exists at the unqualified `members/{member_hash}/installed/{type}/{name}/`, move it to `{install_dir}` before writing (or, if `{install_dir}` already has content, archive the unqualified directory to a timestamped backup) so no stale duplicate is left behind — this piggybacks on the normal resync write and needs no separate migration script. Separately, while the pre-3.6.x legacy layout `members/{member_hash}/{type}/{name}/` still exists on a given install, continue writing to it too to keep it in sync (that single-layout migration is tracked separately and is not in scope here). Specifically write:
   ...
   **5a. Materialize the collection's bundled scripts (added in core 3.29.0).** Once per collection — not once per capability — after the per-capability writes above and before step 8 advances the sync fields:

   - If `aifs_exists("{base}/apps")` is false, skip. Most collections ship no scripts.
   - Otherwise copy `{base}/apps` to `members/{member_hash}/installed/{collection}/apps/`, preserving nested structure, LF-normalizing text content per step 4. Re-resolve the core-injected `apps_path` for this collection's capabilities (`standards.md`, "Core-Injected Parameters"): stored **relative** as `members/{member_hash}/installed/{collection}/apps`, consumed absolute by joining onto the current runtime's `project_dir` at invocation. This is a **wholesale replacement**: remove the local directory first, then write. `apps/` is collection-owned and core-managed; nothing else writes there, so there is nothing to preserve and no merge to perform.
   - `apps/` sits as a sibling of `skill/` and `task/` under `installed/{collection}/`, never inside them, and never at the unqualified legacy paths — the old-unqualified and pre-3.6.x dual-write rules above do not apply to it. `apps/` is a 3.29.0 concept and has no legacy layout to reconcile.
   - On any failure, treat the collection as failed for this run: do not advance `manifest_sync` (step 8), and let the next run retry. A partially materialized `apps/` is worse than none, because setup-time existence gates can pass against it.

   **5b. Record `apps_path` in each affected capability's setup-responses (added in core 3.29.1 — closes `appspathnotbackfilled`).** Materializing the directory without recording the parameter that points at it leaves `{apps_path}` unresolved on every install that predates 3.29.0 — the scripts are present and still uncallable. `org-setup` Phase 4a/4b writes `apps_path` for a *fresh* install; 3.29.0 shipped no equivalent for the *migration* path, so existing members received the files and nothing else. This step is that equivalent.

   For each capability of this collection in `member-index.json`, write `apps_path` into `members/{member_hash}/installed/{collection}/{type}/{name}/setup-responses.md` under the `## Org-Mandated Parameters` section, value `members/{member_hash}/installed/{collection}/apps` — **relative to `project_dir`, never absolute** (`appspathsandboxleak`):

   - **Canonical-format file** (a `## Org-Mandated Parameters` heading exists): add or replace the `### apps_path` block, keeping the `- **Value:**` single-line shape step 9 machine-parses. Leave every other parameter untouched.
   - **Legacy flat-YAML or prose file** (no `##` sections — e.g. `email-triage`'s `delivery_method: chat` shape, or the placeholder `"Installed with org defaults."`): do NOT rewrite it into canonical format here. Converting a member's setup state as a side effect of an unrelated resync is out of scope and risks losing values this step does not understand. Instead append a canonical `## Org-Mandated Parameters` section carrying only `apps_path`. Step 9's parser matches on the heading prefix and will find it; the legacy body above is left exactly as it was.
   - **Idempotent:** if the recorded value already equals the freshly resolved path, write nothing.
   - **Re-resolve, never carry forward.** The recorded value is a record of the last resolution, not an input to the next one (`standards.md`, "Core-Injected Parameters"). Because the stored form is relative it no longer varies by machine or session, but this step still rewrites it on every run so a value left by 3.29.0/3.29.1 — an absolute path naming a dead Cowork sandbox — is replaced rather than preserved.
   - A capability whose collection ships no `apps/` gets nothing — do not write an empty or placeholder `apps_path`.

   Failures here are **not** collection-fatal: the files are already materialized and correct. Surface the capability that failed and continue; the next run retries. This differs deliberately from 5a, where a partial directory can pass an existence gate and mislead.
...
8. **After all capabilities for this collection have been resynced successfully**, set `manifest_sync[<collection>] = coll.version` AND `manifest_sync_subroutine_revision[<collection>] = CURRENT_SUBROUTINE_REVISION` (currently 7) in `member-index.json` and write the file. Both fields advance together; if any capability failed in steps 2–7, step 5a, or step 9 (step 5b is non-fatal — see its own note), do NOT advance either field; the failure leaves them at their prior values, and the next apply-updates run retries.
...
9. **Reconcile setup-responses org-mandated values** (added in core 3.7.6 to close the data half of bug `20260527-8d20ea22-4`). For each capability that was just resynced in steps 2–7, check whether its local `setup-responses.md` org-mandated values match the org's authoritative `collection-setup-responses.md` (read via `aifs_read("/{collection}/setup/collection-setup-responses.md")` — note the corrected path; the pre-3.7.6 `org-setup` spec had this wrong, which is why this reconciliation is needed on every existing install). If drifted: re-inject the org-mandated values into the local setup-responses, preserving member-defined and role-suggested values.
   ...
   **Detection mechanics:**
   ...
   - For each installed capability in this collection (the same set already iterated in steps 2–7):
     ...
     - Parse its `## Org-Mandated Parameters` markdown section (the literal heading written by `org-setup` Phase 4b step 8 is typically `## Org-Mandated Parameters (from collection setup)` — match by the `## Org-Mandated Parameters` prefix, ignoring trailing parenthetical text). Same `### {param_name}` + `- **Value:** {value}` shape as the canonical. If the section is absent — e.g., a placeholder `setup-responses.md` (`"Installed with org defaults."`) produced by an `org-setup` run that couldn't read the canonical at the wrong path; the very bug this step closes — treat the local map as empty: every canonical entry then counts as a missing key (drift).
   ...
   **First-run behavior on existing installs (core 3.29.2).** An install that applied 3.29.1 sits at revision 6 with `apps/` materialized and an *absolute* `apps_path` recorded — in Cowork, one naming a sandbox from a finished session. The bump from 6 to 7 re-sweeps it and step 5b overwrites the value with the relative form. 5a is a cheap no-op (byte-identical re-copy). An install on 3.29.0 gets the 5b write for the first time; one on 3.28.x or earlier gets 5a and 5b together.

   **First-run behavior on pre-3.29.0 installs.** Those have `manifest_sync_subroutine_revision[*] ≤ 4`. The bump from 4 to 6 classifies every installed collection as drifted on the first 3.29.0 apply-updates run, which is exactly the intent: this is how existing members receive `apps/` without reinstalling anything. Steps 1–7 and 9 are no-ops on an already-synced collection; step 5a does the real work, and only for collections that actually ship scripts. On the current install base that is at most four collections, tens of files, a few seconds. Subsequent runs are no-op because the local directory is then present.
...
3. If the member accepts: install capabilities by reading definitions from the **org remote copy** (the admin already placed the collection there via install-collection) — NEVER by re-downloading from GitHub/marketplace, and NEVER by writing collection files to the org remote. Specifically:
   - **Resolve the read base — path-primary, id-anchor fallback (core 3.22.2, J0).** Use `base = "/{collection}"` as primary (the site-drive/path read that works for members on both backends — same as org-setup; see Step 1.5's rationale and the `i1onedrivebreak` correction), with `id:{folder_id}` as the fallback if the path read fails and a `folder_id` is recorded. Read `coll = aifs_read("{base}/collection.json")`. If neither resolves, fall through to the unreadable-base handling below.
