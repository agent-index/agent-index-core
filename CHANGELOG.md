...
## [3.28.0] — 2026-07-18 — Release C.1.5.0: level-3 tooling + collreadgrantmissing + layoutdrift
...
### Added
- **`lib/clone/` — committed clone tooling (level-3).** `clone-repos.{ps1,sh}` + `install-helper-binary.{ps1,sh}` encode the full clone/tag/branch-HEAD/binary logic once (retires `clonescripttagassumption`, `ps1nonascii`). The `clone-script-generator` subroutine now emits only a data manifest; create-org + download-collection invoke the committed script.
- **`lib/permission-spec/build-permission-spec.js` — committed permission-spec builder (level-3).** Validated spec from a data ops-array; enforces the op vocabulary (`transferdocopname`), email/UPN recipient (`recipidform`), and the canonical `outputs/` path (`permspecscratchpad`). Mandated in `permission-change-helper.md`; wired at invite-member + remove-member.

### Fixed
- **`publish-updates` 6e — member-read grant reconcile now covers installed collection roots (`collreadgrantmissing`).** Back-fills the all-members reader grant on every `installed_collections[].folder_id`, not just core + marketplace, so a collection installed without it (e.g. client-intelligence) self-heals on publish. Routed through `build-permission-spec`.
- **`apply-updates` Migration 6 — stale install-dir cleanup (`layoutdrift`).** Removes the pre-`instdir` unqualified `installed/{type}/{name}/` duplicate after positively confirming the collection-qualified superseding copy is present (only member-local migration that deletes files; hard guardrail).
...
## [3.7.4] — 2026-05-24 — "Closing the Loop"
