...
// Canonical roles match Drive's API and aifs_get_permissions output.
const ALLOWED_ROLES = new Set(['reader', 'commenter', 'writer']);
const ALLOWED_MODES = new Set(['fail_soft', 'all_or_nothing']);
const EMAIL_REGEX = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;
// Supported spec versions. 1.0 is the original shape. 1.1 (added in core 3.7.3)
// adds the optional `inherit: boolean` field on share operations.
const SUPPORTED_SPEC_VERSIONS = new Set(['1.0', '1.1']);
const SPEC_VERSION = '1.1';  // current version emitted by tasks built against core 3.7.3+
...
function validate(spec) {
  ...
  // version — accept any supported version. 1.0 specs validate without `inherit`;
  // 1.1 specs may include `inherit` on share operations.
  if (!SUPPORTED_SPEC_VERSIONS.has(spec.version)) {
    errors.push(`spec.version must be one of: ${[...SUPPORTED_SPEC_VERSIONS].join(', ')} (got: ${JSON.stringify(spec.version)})`);
  }
  ...
  if (!Array.isArray(spec.operations)) {
    ...
    spec.operations.forEach((op, i) => validateOperation(op, i, errors, spec.version));
...
function validateOperation(op, i, errors, specVersion) {
  ...
  // inherit is optional, share-ops-only, boolean. Added in core 3.7.3 (spec v1.1).
  // Default `true` when omitted (matches v1.0 / pre-3.7.3 behavior). Setting `false`
  // requires the adapter to apply override semantics — for gdrive that means
  // `inheritedPermissionsDisabled: true` on the file, requiring organizer role.
  if (op.inherit !== undefined) {
    if (specVersion === '1.0') {
      errors.push(`${prefix}.inherit requires spec.version "1.1" or higher (got spec version: ${JSON.stringify(specVersion)})`);
    } else if (op.op !== 'share') {
      errors.push(`${prefix}.inherit is only valid on 'share' operations (got op: ${JSON.stringify(op.op)})`);
    } else if (typeof op.inherit !== 'boolean') {
      errors.push(`${prefix}.inherit must be a boolean (got: ${JSON.stringify(op.inherit)})`);
    }
  }
    ...
    if (op.before === null || typeof op.before !== 'object') {
      ...
      op.before.recipients.forEach((r, j) => {
        if (!r || typeof r !== 'object') {
          errors.push(`${prefix}.before.recipients[${j}] must be an object.`);
          return;
        }
        // Canonical recipient identifier is "subject" (matches Drive API +
        // aifs_get_permissions output).
        if (typeof r.subject !== 'string' || !r.subject || typeof r.role !== 'string' || !r.role) {
          errors.push(`${prefix}.before.recipients[${j}] must have non-empty subject and role.`);
        }
      });
    }
  }
